const AIQuestion = {
    ...
    // =====================================================
    // ✅ 新增：调休上班日列表（周末但需要上班的日期）
    // 格式：YYYY-MM-DD
    // 这些日期即使是周六/周日，也要计入次数限制
    // =====================================================
    WORKDAYS: [
        // ===== 2025 年调休上班 =====
        '2025-01-26',  // 周日（春节调休）
    ...
    // =====================================================
    // ✅ 新增：判断今天是否法定节假日（含调休放假日）
    // =====================================================
    isHoliday() {
        return this.HOLIDAYS.includes(this.getTodayStr());
    },


    // =====================================================
    // ✅ 新增：判断今天是否调休上班日（周末但需要上班）
    // =====================================================
    isMakeupWorkday() {
        return this.WORKDAYS.includes(this.getTodayStr());
    },
    ...
    // ✅ 新增：判断今天是否「不限次数」
    // 逻辑：
    //   1. 先看是否是调休上班日 → 是的话，即使周末也限制
    //   2. 再看是否是法定节假日 → 是的话，即使工作日也不限
    //   3. 最后看是否是周末 → 是的话不限
    // =====================================================
    isUnlimitedDay() {
        // 调休上班日优先：周末但要上班，按工作日处理
    ...
    // =====================================================
    increaseDailyCount() {
        if (this.isUnlimitedDay()) return;  // 周末 / 节假日 不计数
    ...
    updateLimitDisplay() {
        ...
        if (this.isMakeupWorkday()) {
            ...
            if (remaining > 0) {
                el.textContent =
                    `今日调休上班，剩余 ${remaining} 次（工作日每天 ${this.DAILY_LIMIT} 次）`;
                el.style.color = '#64748b';
            } else {
                el.textContent = '今日调休上班，次数已用完';
        ...
        if (remaining > 0) {
            el.textContent =
                `今日剩余 ${remaining} 次（工作日每天 ${this.DAILY_LIMIT} 次，周末及节假日不限）`;
            el.style.color = '#64748b';
        } else {
            el.textContent = '今日次数已用完（周末及节假日不限）';
            el.style.color = '#dc2626';
        }
    },
